...
- **Manual-override respect.** If you manually turn a climate entity back on after the automation has shut it off, the blueprint will **not** turn it off again on subsequent triggers in the same open-window cycle. Detection compares each climate's `last_changed` against the per-zone snapshot's `last_changed`: any non-off climate that changed after the snapshot is assumed to be a user override and is excluded from the turn-off list. When all affected climates are already off or overridden, the path is skipped entirely (no redundant `climate.turn_off`, no notification).
